...
public:
    void reorderList(ListNode* head) {
        ...
        ListNode* prev=NULL;   //4->5->NULL to NULL<-4<-5
        while(second){ // isse second half reverse ho jayegi
            ListNode* front=second->next; // stores 5
            second->next=prev; // second apna 4 hai to 4 points to prev ie 4->NULL
            prev=second; // moving pointers second ab hamara prev hai
            second=front; // front ab hamara second hai 
            // prev-second-front yeh order hai
        }
        // now time to merge
        ListNode* first=head;
        second=prev;
